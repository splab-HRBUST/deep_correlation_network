...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        avg_out = torch.mean(x, dim=1, keepdim=True)#按行求平均值
...
class ResNet(nn.Module):
    ...
    def forward(self, x): #x_0 =  torch.Size[32,192,251]
...
class SpectrogramModel(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        resnet = resnet18_cbam
        model_res = resnet(False).to("cuda:1")#将resnet18_cbam模型也转移到GPU，否则会报错
        x = model_res(x)
