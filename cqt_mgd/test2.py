...
class cqt_mgd(nn.Module):
    def __init__(self, block, layers, num_classes, emb_dim1,emb_dim2,T,Q,
    ...
    def get_em_param(self,x,y,device):#x:cqt(512,32) y:mgd(512,32) T:(512,32) Q:(512,32)
        ...
        variancesY =torch.sum(centeredY**2,1) / N2 #(512)
        ...
        B=torch.transpose(self.T,dim0=0,dim1=1)/torch.transpose(self.fai_x,dim0=0,dim1=1)#(256,512)
        C=torch.transpose(self.Q,dim0=0,dim1=1)/torch.transpose(self.fai_y,dim0=0,dim1=1)#(256,512)
        
        L=I+B@self.T+C@self.Q #(32,32)#(256,256)
       
            
        cov_eps = torch.eye(L.shape[1]) * (1e-12)
        cov_eps = cov_eps.to(device)

        cov=torch.linalg.pinv(L+cov_eps)#(256,256)

        E1=cov@(B@centeredM+C@centeredY)#(256,32) E1为隐变量w
        ...
        T=(centeredM@E1_T)@torch.linalg.pinv(E2+cov_eps)#(512,32)
 
        Q=(centeredY@E1_T)@torch.linalg.pinv(E2+cov_eps)#(512,32)
...
if __name__ == '__main__':
    ...
    model = cqt_mgd(BasicBlock, [2, 2, 2, 2],num_classes=2,emb_dim1=512,emb_dim2=256,T=a,Q=b)
    ...
    #loss2=model.loss_em2(z,mu_z,fai_z,lamda,E1,E2,device)
