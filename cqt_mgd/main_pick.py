...
def evaluate_accuracy(data_loader, model, device):
    ...
    with torch.no_grad():
        ...
        for data in data_loader:
            ...
            #print("eval2")
            x,y,batch_out = model(batch_x1,batch_x2)
            #print("eval3")
...
def train_epoch(data_loader, model, device):
    ...
    for data in data_loader:
        ...
        len = batch_y1.shape[0]
        cat_scores(batch_score,batch_y1,len)
        #两个特征的标签相同，所以采用其中一个即可
        loss = criterion_pretrain(batch_out,batch_y1)
...
if __name__ == '__main__':
    ...
    if not os.path.exists('initialization'):
    ...
    model_save_path = os.path.join('initialization', model_tag) #模型保存路径
...
#----------------------------------------------------------------------------------------------------
    ...
    dev_loader = DataLoader(myDevset, batch_size=args.batch_size, shuffle=True,pin_memory=True)
#----------------------------------------------------------------------------------------------
    start_epoch=0
...
#-----------------------------------------------------------------------------------
    ...
    train_loader = DataLoader(
        myTrainset, batch_size=args.batch_size, num_workers=num_workers,shuffle=True,pin_memory=True)
