...
def train_epoch(data_loader,model,device,lr):
    ...
    for data in data_loader:
        ...
        loss=0.001*em_loss+cross_loss#将em_loss的量纲更改成同cross_loss l=0.1
...
if __name__ == '__main__':
    ...
    if not os.path.exists('s2s3_0.001model'):
        os.mkdir('s2s3_0.001model')
    ...
    model_save_path = os.path.join('s2s3_0.001model', model_tag) #模型保存路径
...
#-------------------------------------------------------------------------------------------------
    ...
    df.to_csv("s2s3_0.001.csv",index=False)
    for epoch in range(start_epoch+1,num_epochs+1):
        ...
        data.to_csv("s2s3_0.001.csv",mode='a',header=False,index=False)#mode设为a,就可以向csv文件追加数据了
