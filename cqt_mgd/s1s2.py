...
from test5 import cqt_mgd,em_param
#from test6 import cqt_mgd,em_param
...
def train_epoch(data_loader,model,device,lr):
    ...
    num_total = 0.0
    i = 0
    #optim1 = torch.optim.Adam(model.parameters(), lr=lr,amsgrad=True) 
    optim1 = torch.optim.Adam(model.parameters(), lr=lr) # 优化算法
...
if __name__ == '__main__':
    ...
    if not os.path.exists('s1s2_0.00001model'):
        os.mkdir('s1s2_0.00001model')
    ...
    model_save_path = os.path.join('s1s2_0.00001model', model_tag) #模型保存路径
...
#-------------------------------------------------------------------------------------------------
    ...
    df.to_csv("s1s2_0.00001.csv",index=False)
    for epoch in range(start_epoch+1,num_epochs+1):
        ...
        data.to_csv("s1s2_0.00001.csv",mode='a',header=False,index=False)#mode设为a,就可以向csv文件追加数据了
